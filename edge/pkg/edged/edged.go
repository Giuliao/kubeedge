...
func (e *edged) Start(c *context.Context) {
	...
	e.statusManager = status.NewManager(e.kubeClient,
		e.podManager,
		utilpod.NewPodDeleteSafety(),
		e.metaClient)
	...
	e.volumeManager = volumemanager.NewVolumeManager(false,
	...
	e.probeManager = prober.NewManager(e.statusManager,
		e.livenessManager,
		containers.NewContainerRunner(),
		kubecontainer.NewRefManager(),
		record.NewEventRecorder())

	e.pleg = edgepleg.NewGenericLifecycleRemote(e.containerRuntime,
		e.probeManager,
		plegChannelCapacity,
		plegRelistPeriod,
		e.podManager,
...
func getConfig() *Config {
	...
	conf.DockershimRootDirectory = config.CONFIG.GetConfigurationByKey("edged.dockershim-root-diretcory").(string)
	conf.DockerDisableSharedPID = config.CONFIG.GetConfigurationByKey("edged.docker-disable-shared-PID").(bool)
	conf.CNIBinDir = config.CONFIG.GetConfigurationByKey("edged.cni-bin-dir").(string)
	conf.CNIConfDir = config.CONFIG.GetConfigurationByKey("edged.cni-conf-dir").(string)
	conf.CNIName = config.CONFIG.GetConfigurationByKey("edged.cni-name").(string)
	conf.KubeCgroupDriver = config.CONFIG.GetConfigurationByKey("edged.cgroup-driver").(string)
...
func getRuntimeAndImageServices(remoteRuntimeEndpoint string, remoteImageEndpoint string,
...
func newEdged() (*edged, error) {
	...
	case DockerContainerRuntime:
		dockerclient := &dockershim.ClientConfig{
			DockerEndpoint:            conf.DockerAddress,
			RuntimeRequestTimeout:     conf.RuntimeRequestTimeout.Duration,
			ImagePullProgressDeadline: 10,
		}

		pluginSettings := dockershim.NetworkPluginSettings{
			PluginName:    conf.CNIName,
			PluginBinDir:  conf.CNIBinDir,
			PluginConfDir: conf.CNIConfDir,
		}
		...
		ds, err := dockershim.NewDockerService(dockerclient, conf.PodSandboxImage, streamingConfig, &pluginSettings,
			conf.runtimeType, conf.KubeCgroupDriver, conf.DockershimRootDirectory, conf.DockerDisableSharedPID)
		if err != nil {
			return nil, err
		}
